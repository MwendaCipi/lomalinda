...
class MpesaInitiationAPITests(APITestCase):
    ...
    @patch('members.views.initiate_stk_push_for_context')
    def test_a_split_gift_names_the_prompt_after_its_account_count(self, mock_stk):
        """One reference per push: a split gift says how many accounts it feeds.

        Naming the prompt after the first account made a two-account gift read
        as if the whole sum were going to that one account.
        """
        mock_stk.return_value = {'CustomerMessage': 'Prompt sent.'}
        response = self.client.post('/api/members/contributions/initiate/', {
            'giving_type': 'financial',
            ...
            'phone_number': '0712345678',
            'allocations': [
                {'purpose': 'Tithe', 'account': 'TITHE', 'amount': '100.00'},
                {'purpose': 'Local Church Budget', 'account': 'LCB', 'amount': '200.00'},
            ],
        }, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        kwargs = mock_stk.call_args.kwargs
        self.assertEqual(kwargs['amount'], Decimal('300.00'))
        self.assertEqual(kwargs['purpose'], '2accounts')
        self.assertEqual(account_reference_for_purpose(kwargs['purpose']), '2ACCOUNTS')
        # The split itself still travels to the callback, per account.
        self.assertEqual(
            [row['account'] for row in unpack_callback_context(kwargs['context_token'])['allocations']],
            ['TITHE', 'LCB'],
        )
