...
/** Mobile replacement for the desktop sidebar: sections rendered as tappable cards. */
export function MaterialsMobileCards() {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const raw = searchParams.get("section") as MaterialSection | null;
  const isValidSection = raw && materialSections.some((s) => s.value === raw);
  const active: MaterialSection = isValidSection
    ? (raw as MaterialSection)
  ...
  return (
    <div className="mb-5 lg:hidden">
      <p className="mb-2 text-[10px] font-extrabold uppercase tracking-wider text-[#b36b3c]">
        Study Materials
      </p>
          ...
          return (
            <Link
              ...
              className={`flex items-start gap-3 rounded-2xl border p-4 shadow-sm transition ${
                isActive
                  ? "border-[#b36b3c] bg-[#b36b3c]/5 ring-1 ring-[#b36b3c]"
            ...
            >
              ...
              >
                <Icon className={`h-4 w-4 ${isActive ? "text-[#b36b3c]" : "text-[#617068]"}`} />
              </span>
              <span className="min-w-0">
                <span className="block text-sm font-bold text-[#26352f]">{section.label}</span>
                <span className="mt-0.5 block text-[11px] leading-relaxed text-[#617068]">
                  {section.description}
                </span>
              </span>
            </Link>
