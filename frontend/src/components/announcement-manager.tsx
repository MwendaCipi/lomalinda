    ...
    <section className="rounded-2xl border border-[#dfdbd1] bg-white p-7">
      ...
      <form onSubmit={submit} className="mt-6 grid gap-5 md:grid-cols-2">
        <label className="block text-sm font-medium">Title
          <input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} className="mt-2 w-full rounded-xl border border-[#c9c5bb] px-4 py-3" />
        </label>
        <label className="block text-sm font-medium">Visibility
          <select value={form.visibility} onChange={(event) => setForm({ ...form, visibility: event.target.value })} className="mt-2 w-full rounded-xl border border-[#c9c5bb] bg-white px-4 py-3">
        ...
        <div className="md:col-span-2">
          ...
          {message && <p className="mt-3 text-sm text-[#617068]">{message}</p>}
        </div>
      </form>
    </section>
  );
