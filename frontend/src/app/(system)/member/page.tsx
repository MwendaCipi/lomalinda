      ...
      <div className="flex h-full md:h-[calc(100vh-4rem)] md:overflow-hidden">
        ...
        <div className="flex-1 min-w-0 h-full p-4 sm:p-8 lg:p-10 overflow-y-auto custom-hover-scrollbar">
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#b36b3c]">Member space</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Your giving history</h1>
            </div>
            <Link href="/give" className="rounded-full bg-[#b36b3c] px-5 py-2.5 text-xs sm:text-sm font-semibold text-white hover:bg-[#96552e]">
              Give now
            </Link>
          </div>

          <div className="mt-4 flex flex-wrap gap-4 text-xs sm:text-sm">
            <Link href="/member/reports" className="font-semibold text-[#b36b3c]">View church financial reports &rarr;</Link>
            <Link href="/community/welfare" className="font-semibold text-[#b36b3c]">Church welfare &rarr;</Link>
            <Link href="/announcements" className="font-semibold text-[#b36b3c]">Member announcements &rarr;</Link>
          </div>

          {details && (
            <form onSubmit={saveDetails} className="mt-8 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfdbd1]">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="block text-sm font-medium">
                  Date of birth
                  <input type="date" value={details.date_of_birth || ""} onChange={(event) => setDetails({ ...details, date_of_birth: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[#c9c5bb] px-4 py-2.5" />
                </label>
                <label className="block text-sm font-medium">
                  County of birth
                  <select value={details.county_of_birth || ""} onChange={(event) => setDetails({ ...details, county_of_birth: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[#c9c5bb] px-4 py-2.5">
                ...
                <label className="block text-sm font-medium">
                  Level of education
                  <input value={details.education_level || ""} onChange={(event) => setDetails({ ...details, education_level: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[#c9c5bb] px-4 py-2.5" />
                </label>
                <label className="block text-sm font-medium">
                  Profession
                ...
                {details.current_church !== undefined && (
                  <label className="block text-sm font-medium sm:col-span-2">
                    Current church
                    <input value={details.current_church || ""} onChange={(event) => setDetails({ ...details, current_church: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[#c9c5bb] px-4 py-2.5" />
                  </label>
                )}
              </div>
              <button disabled={savingDetails} className="mt-5 rounded-full bg-[#5f8067] px-5 py-3 text-sm font-semibold text-white disabled:opacity-60">
          ...
          <div className="mt-8 space-y-4">
            {contributions.map((item) => (
              <article key={item.id} className="flex flex-col gap-3 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-[#dfdbd1] sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="font-semibold">{item.purpose}</p>
                  <p className="mt-1 text-sm text-[#617068]">{new Date(item.created_at).toLocaleDateString()}</p>
                </div>
                <div className="sm:text-right">
                  <p className="text-xl font-semibold">{item.currency} {Number(item.amount).toLocaleString()}</p>
                  <p className="mt-1 text-sm capitalize text-[#617068]">{item.status}</p>
                </div>
              </article>
            ))}
            {message && (
              <div className="rounded-2xl bg-white p-6 text-[#617068] shadow-sm ring-1 ring-[#dfdbd1]">
