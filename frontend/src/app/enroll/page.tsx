...
export default function EnrollPage() {
  ...
  async function submitViaEmail(event?: FormEvent) {
    if (event) event.preventDefault();
    if (!form.first_name.trim() || !form.surname.trim() || !form.phone_number.trim() || !form.email.trim()) {
      showAlert("Missing information", "Please fill in your name, phone number, and email address.", "warning");
      return;
    }
    if (joiningMode === "friend" && !form.current_church.trim()) {
      showAlert("Missing information", "Please specify the name of the church you currently attend.", "warning");
      return;
    }
    setLoading(true);
    setMessage("");
  ...
  async function submitTransferOut(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = {
        member_name: `${form.first_name} ${form.surname}`.trim(),
        transfer_type: "outgoing",
        other_church: form.destination_church,
        reason: form.transfer_reason,
  ...
  return (
    <main className="flex min-h-screen items-start justify-center bg-[#f7f4ee] px-6 pb-16 pt-8 text-[#26352f] sm:py-10">
      <section className="w-full max-w-md rounded-3xl bg-white p-5 shadow-sm ring-1 ring-[#dfdbd1] sm:p-7 lg:max-w-3xl">
        <Link href="/requests" className="mb-4 inline-flex items-center gap-2 text-xs font-semibold text-[#b36b3c]">
          <span>&larr;</span>
        ...
        <p className="mt-2 text-sm leading-6 text-[#617068]">
          Choose whether you would like to join us or request a transfer out.
        </p>
        <div className="mt-5 flex gap-1 rounded-2xl border border-[#dfdbd1] p-1">
          <button
            ...
            className={`flex-1 rounded-xl px-3 py-2.5 text-sm font-semibold ${
              tab === "join" ? "bg-[#5f8067] text-white shadow-sm" : "text-[#617068] hover:bg-[#f7f4ee]"
            }`}
          >
            Join us
          </button>
          <button
            type="button"
            onClick={() => selectTab("transfer_out")}
            className={`flex-1 rounded-xl px-3 py-2.5 text-sm font-semibold ${
              tab === "transfer_out" ? "bg-[#9a741c] text-white shadow-sm" : "text-[#617068] hover:bg-[#f7f4ee]"
            }`}
          >
            Transfer out
          </button>
        ...
        {tab === "join" ? (
          <form onSubmit={submitViaEmail} className="mt-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block text-sm font-medium">
                ...
                >
                  ...
                  <option value="friend">Friend of Loma Linda</option>
                </select>
              </label>
              <label className="block text-sm font-medium">
                First name
              ...
              {joiningMode === "friend" && (
                <label className="block text-sm font-medium sm:col-span-2">
                  ...
                  <input
                    ...
                    className={inputClass}
                    placeholder="Name of the church you currently attend"
                  />
                </label>
              )}
            </div>
          ...
          <form onSubmit={submitTransferOut} className="mt-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block text-sm font-medium">
                First name
                <input
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <button
                disabled={loading}
                className="inline-flex h-11 w-full items-center justify-center rounded-full bg-[#9a741c] px-5 font-medium text-white transition hover:bg-[#7c5d16] disabled:opacity-60 sm:col-start-2"
              >
