...
export default function MinistryDetailClient() {
  ...
  if (!ministry) return <main className="min-h-screen bg-[#f7f4ee] px-6 py-12 text-center text-[#26352f]"><h1 className="text-2xl font-semibold">Ministry not found</h1><Link href="/ministries" className="mt-4 inline-block font-semibold text-[#b36b3c]">View all ministries &rarr;</Link></main>;
  return <main className="min-h-screen bg-[#f7f4ee] text-[#26352f]"><div className="mx-auto max-w-5xl px-6 py-6 lg:px-8 lg:py-10"><div className="max-w-3xl"><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">{ministry.title}</h1><p className="mt-3 text-base leading-7 text-[#617068] sm:text-lg">{ministry.description}</p></div>{ministry.sections && <div className="mt-8 grid gap-5 md:grid-cols-3">{ministry.sections.map((section) => <section id={section.id} key={section.title} className="scroll-mt-28 rounded-2xl border border-[#dfdbd1] bg-white p-6 sm:p-7"><h2 className="text-lg font-semibold sm:text-xl">{section.title}</h2><p className="mt-3 text-sm leading-6 text-[#617068]">{section.text}</p></section>)}</div>}{ministry.department && <section className="mt-8"><h2 className="text-xl font-semibold sm:text-2xl">{ministry.title} calendar</h2><DepartmentCalendar department={ministry.department} events={events} loaded={loaded} /><p className="mt-6 text-xs text-[#617068]">Showing events published for {year}.</p></section>}</div></main>;
}
