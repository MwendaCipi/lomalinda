...
export default function MinistriesPage() {
  return <main className="min-h-screen bg-[#f7f4ee] text-[#26352f]"><div className="mx-auto max-w-5xl px-6 py-12 lg:px-8 lg:py-16"><div className="max-w-3xl"><h1 className="text-4xl font-semibold tracking-tight sm:text-6xl">Our ministries</h1><p className="mt-5 text-lg leading-8 text-[#617068]">Find a place to worship, serve, learn, and grow with the Loma Linda church family.</p></div><div className="mt-12 grid gap-5 md:grid-cols-2">{ministries.map((ministry) => <Link key={ministry.slug} href={`/ministries/${ministry.slug}`} className="rounded-2xl border border-[#dfdbd1] bg-white p-7 transition hover:-translate-y-0.5 hover:border-[#b36b3c] hover:shadow-sm"><h2 className="text-2xl font-semibold">{ministry.title}</h2><p className="mt-3 text-sm leading-6 text-[#617068]">{ministry.description}</p><span className="mt-6 inline-block text-sm font-semibold text-[#b36b3c]">Explore {ministry.title} &rarr;</span></Link>)}</div></div></main>;
}
