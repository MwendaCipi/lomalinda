...
function CalendarPageContent() {
  ...
  return <main className="min-h-screen bg-[#f7f4ee] px-6 pb-10 pt-4 text-[#26352f] sm:pb-16 sm:pt-6"><div className="mx-auto max-w-6xl"><div className="grid gap-4 py-5 md:grid-cols-[150px_170px_1fr]"><label className="text-sm font-semibold">Year<select value={selectedYear} onChange={(event) => setSelectedYear(Number(event.target.value))} className="mt-2 block w-full rounded-lg border border-[#cfc9bd] bg-white px-3 py-2 font-normal">{years.map((year) => <option key={year} value={year}>{year}</option>)}</select></label><label className="text-sm font-semibold">Month<select value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)} className="mt-2 block w-full rounded-lg border border-[#cfc9bd] bg-white px-3 py-2 font-normal"><option value="all">All months</option>{monthNames.map((month, index) => <option key={month} value={index}>{month}</option>)}</select></label><div className="flex items-end gap-3"><label className="min-w-0 flex-1 text-sm font-semibold">Search<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Event, department, or date" className="mt-2 block w-full rounded-lg border border-[#cfc9bd] bg-white px-3 py-2 font-normal outline-none focus:border-[#b36b3c]" /></label><button type="button" onClick={() => window.print()} className="shrink-0 rounded-lg bg-[#b36b3c] px-4 py-2 text-sm font-semibold text-white hover:bg-[#96552e]">Print Calendar</button></div></div><div className="mt-8 overflow-x-auto rounded-xl border border-[#dfdbd1] bg-white"><table className="w-full min-w-[760px] border-collapse text-left text-sm"><thead className="border-b border-[#dfdbd1] bg-[#eef2ed] text-xs uppercase tracking-[0.12em] text-[#617068]"><tr><th className="px-5 py-4 font-semibold">Date</th><th className="px-5 py-4 font-semibold">Event</th><th className="px-5 py-4 font-semibold">Time</th><th className="px-5 py-4 font-semibold">Department</th><th className="px-5 py-4 font-semibold">Action</th></tr></thead><tbody className="divide-y divide-[#e9e5dd]">{rows.map((row) => { const actionKey = `${row.date}-${row.event.name}`; return <tr key={actionKey} className="hover:bg-[#fcfbf9]"><td className="whitespace-nowrap px-5 py-4 text-[#617068]">{new Date(`${row.date}T12:00:00`).toLocaleDateString("en-KE", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</td><td className="px-5 py-4 font-semibold">{row.event.name}</td><td className="whitespace-nowrap px-5 py-4 text-[#617068]">{row.event.time || "-"}</td><td className="px-5 py-4 text-[#617068]">{row.event.department || "-"}</td><td data-calendar-action-menu className="relative px-5 py-4"><button type="button" aria-expanded={openActions === actionKey} onClick={() => setOpenActions(openActions === actionKey ? null : actionKey)} className="rounded-lg border border-[#c9c5bb] px-3 py-2 text-sm font-semibold text-[#26352f] hover:border-[#b36b3c]">Actions <span aria-hidden="true">v</span></button>{openActions === actionKey && <div className="absolute right-5 top-14 z-20 w-48 rounded-xl border border-[#dfdbd1] bg-white p-2 shadow-lg">{row.event.kind === "online" && row.event.meeting_link && <a href={row.event.meeting_link} target="_blank" rel="noreferrer" className="block rounded-lg px-3 py-2 text-sm hover:bg-[#f7f4ee]">Join meeting</a>}{row.event.location_link && <a href={row.event.location_link} target="_blank" rel="noreferrer" className="block rounded-lg px-3 py-2 text-sm hover:bg-[#f7f4ee]">Open map</a>}<Link href={`/give?purpose=${encodeURIComponent(row.event.name)}`} onClick={() => setOpenActions(null)} className="block rounded-lg px-3 py-2 text-sm hover:bg-[#f7f4ee]">Give support</Link><a href={`mailto:hello@lomalindachurch.org?subject=${encodeURIComponent(`Contact leader: ${row.event.name}`)}`} onClick={() => setOpenActions(null)} className="block rounded-lg px-3 py-2 text-sm hover:bg-[#f7f4ee]">Contact department</a><a href={`mailto:hello@lomalindachurch.org?subject=${encodeURIComponent(`Suggestion: ${row.event.name}`)}`} onClick={() => setOpenActions(null)} className="block rounded-lg px-3 py-2 text-sm hover:bg-[#f7f4ee]">Give suggestion</a><button type="button" onClick={() => { setOpenActions(null); openProgram(row); }} className="block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-[#f7f4ee]">View program</button></div>}</td></tr>; })}</tbody></table>{loaded && rows.length === 0 && <p className="px-5 py-10 text-center text-sm text-[#617068]">No calendar entries match your filters.</p>}</div>{!loaded && <p className="mt-8 text-sm text-[#617068]">Loading the church calendar...</p>}<p className="mt-4 text-xs text-[#617068]">Showing {rows.length} {rows.length === 1 ? "entry" : "entries"}.</p></div><SabbathProgramModal program={activeProgram} onClose={() => setActiveProgram(null)} /></main>;
}
