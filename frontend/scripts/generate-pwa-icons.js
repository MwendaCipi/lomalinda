  ...
  return `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <g transform="translate(${safePadding}, ${safePadding})">
        <!-- Outer circle keeps the emblem and full church name together -->
        <circle cx="${innerW / 2}" cy="${innerH / 2}" r="${innerW * 0.45}" fill="#1d2a25" stroke="#b36b3c" stroke-width="${innerW * 0.015}" opacity="0.9" />

        <!-- White Flame & Cross SDA Symbol -->
        <g transform="translate(${innerW * 0.28}, ${innerH * 0.16}) scale(${innerW * 0.0017})">
        ...
        <!-- Clean Typography "SDA Church Lomalina, Meru" Safely Below the Circle -->
        <text x="${innerW / 2}" y="${innerH * 0.70}" font-family="sans-serif" font-weight="800" font-size="${innerW * 0.08}" fill="#b36b3c" text-anchor="middle" letter-spacing="1">SDA</text>
        <text x="${innerW / 2}" y="${innerH * 0.81}" font-family="sans-serif" font-weight="700" font-size="${innerW * 0.055}" fill="#FFFFFF" text-anchor="middle" letter-spacing="0.5">CHURCH LOMALINA</text>
        <text x="${innerW / 2}" y="${innerH * 0.90}" font-family="sans-serif" font-weight="600" font-size="${innerW * 0.06}" fill="#b36b3c" text-anchor="middle" letter-spacing="1">MERU</text>
      </g>
    </svg>
  `;
}
...
async function generate() {
  ...
  console.log('SDA Church Lomalina, Meru PWA Icons updated successfully!');
}
