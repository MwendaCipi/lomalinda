...
// Create an SVG icon badge featuring the Loma Linda SDA Church emblem
  ...
  return `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <g transform="translate(${safePadding}, ${safePadding})">
        <!-- Golden accent ring -->
        <circle cx="${innerW / 2}" cy="${innerH / 2}" r="${innerW * 0.42}" fill="none" stroke="#b36b3c" stroke-width="${innerW * 0.02}" opacity="0.6" />
        <!-- White Flame & Cross Symbol -->
        <g transform="translate(${innerW * 0.22}, ${innerH * 0.18}) scale(${innerW * 0.0022})">
        ...
        <text x="${innerW / 2}" y="${innerH * 0.82}" font-family="sans-serif" font-weight="bold" font-size="${innerW * 0.085}" fill="#FFFFFF" text-anchor="middle">LOMA LINDA</text>
        <text x="${innerW / 2}" y="${innerH * 0.91}" font-family="sans-serif" font-weight="500" font-size="${innerW * 0.055}" fill="#b36b3c" text-anchor="middle">SDA CHURCH</text>
      </g>
    </svg>
  `;
}
