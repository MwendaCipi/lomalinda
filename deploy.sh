...
# New imports arrive with the code, and a missing one stops gunicorn from
# booting at all - so dependencies are installed before anything is restarted.
# Idempotent: an unchanged requirements.txt is a no-op.
